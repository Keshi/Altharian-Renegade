...
// End of Spell Cast Hook
    ...
    effect eAttack = EffectAttackIncrease(nMod);
    effect eDamage = EffectDamageIncrease(nMod, DAMAGE_TYPE_SLASHING);
    ...
    while (GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF)
        {
           ...
           if(!MyResistSpell(OBJECT_SELF, oTarget)  && !MySavingThrow(SAVING_THROW_WILL, oTarget, (GetSpellSaveDC()+nMod), SAVING_THROW_TYPE_FEAR))
            {
                DelayCommand(0.01, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink2, oTarget, RoundsToSeconds(2*nMod)));
            }
        }
