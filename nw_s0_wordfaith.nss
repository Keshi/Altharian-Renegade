...
// End of Spell Cast Hook
    ...
    int nPriest = GetLevelByClass (CLASS_TYPE_CLERIC,OBJECT_SELF) +  GetLevelByClass (CLASS_TYPE_PALADIN,OBJECT_SELF) +  GetLevelByClass (CLASS_TYPE_DIVINECHAMPION,OBJECT_SELF);
    float fSize = RADIUS_SIZE_COLOSSAL;
    if (nVesper == 2 || nVesper == 4)
      {
        nLevel = GetEffectiveCasterLevel(OBJECT_SELF);
        if (nLevel > nPriest) nPriest = nLevel;
        nMod = nPriest / 5;
        ...
        while (GetIsObjectValid(oTarget))
        {
            ...
            if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF))
            {
                ...
                if(!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
                {
                    ...
                    int nDamage = d10(nPriest);
                    effect eBlast= EffectDamage(nDamage,DAMAGE_TYPE_DIVINE);
