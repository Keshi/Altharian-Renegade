...
void main()
{
    ...
    if (nVesper >= 2)
      {
        nMod = GetEffectiveLevel(OBJECT_SELF);
        nTurnLevel = nTurnLevel/nTotalLevel * nMod;
        nMod = nTurnLevel/10;
        if (nMod < 2) nMod = 2;
      }
    ...
    int nTurnCheck = d20() + nChrMod + nMod;              //The roll to apply to the max HD of undead that can be turned --> nTurnLevel
    int nTurnHD = d20(nMod) + nChrMod + nTurnLevel;   //The number of HD of undead that can be turned.

    if(nSun == TRUE)
    {
        nTurnCheck += d4(nMod-1);
        nTurnHD += d6(nMod-1);
    }
    ...
    while(GetIsObjectValid(oTarget) && nHDCount < nTurnHD && GetDistanceToObject(oTarget) <= 20.0)
    {
        if(!GetIsFriend(oTarget))
        {
            ...
            if (nRacial == RACIAL_TYPE_OUTSIDER )
            {
                if (nPlanar)
                {
                     //Planar turning decreases spell resistance against turning by Modifier
                     nHD = GetHitDice(oTarget) + (GetSpellResistance(oTarget) /nMod) + GetTurnResistanceHD(oTarget);
                }
                else
                {
            ...
            if(nHD <= nTurnLevel && nHD <= (nTurnHD - nHDCount))
            {
                ...
                else if (nRacial == RACIAL_TYPE_CONSTRUCT && nConstructs > 0)
                {
                    ...
                    if (nVesper >= 2) nDamage = d3(nTurnLevel) + (nMod * nTurnLevel);
