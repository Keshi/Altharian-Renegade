...
void main()
{
    ...
    effect eAcid = EffectDamage(d10(nMod), DAMAGE_TYPE_ACID);
    effect eElec = EffectDamage(d10(nMod*2), DAMAGE_TYPE_ELECTRICAL);
    ...
    while(GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, GetAreaOfEffectCreator()))
        {
            ...
            if(MyResistSpell(GetAreaOfEffectCreator(), oTarget, fDelay) == 0)
            {
                ...
                if(MySavingThrow(SAVING_THROW_REFLEX, oTarget, (GetSpellSaveDC() + (nMod/2)), SAVING_THROW_TYPE_ELECTRICITY, GetAreaOfEffectCreator(), fDelay))
                {
