...
void RunImpact(object oTarget, object oCaster, int nMetaMagic, int nBase, int nNumDice);
...
void main()
{
    ...
    if (GetIsReactionTypeFriendly(oTarget) == FALSE)
    {
        ...
        {
            //----------------------------------------------------------------------
            // Do the initial points of damage
            //----------------------------------------------------------------------
            int nDamage = MaximizeOrEmpower(nDice,nMax,nMetaMagic);
            ...
            DelayCommand(6.0f,RunImpact(oTarget, oSelf, nMetaMagic, nDice, nMax));
        }
        else
        {
            //----------------------------------------------------------------------
...
void RunImpact(object oTarget, object oCaster, int nMetaMagic, int nBase, int nNumDice)
{
    ...
    if (GetIsDead(oTarget) == FALSE)
    {
        ...
        // Calculate Damage
        //----------------------------------------------------------------------
        int nDamage = MaximizeOrEmpower(nBase,nNumDice,nMetaMagic);
        effect eDam = EffectDamage(nDamage, DAMAGE_TYPE_ACID);
        effect eVis = EffectVisualEffect(VFX_IMP_ACID_S);
        eDam = EffectLinkEffects(eVis,eDam); // flare up
        ApplyEffectToObject (DURATION_TYPE_INSTANT,eDam,oTarget);
        DelayCommand(6.0f,RunImpact(oTarget,oCaster,nMetaMagic,nBase,nNumDice));
    }
}
