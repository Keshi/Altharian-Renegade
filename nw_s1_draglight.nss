...
void main()
{
    ...
    effect ePos;
     ...
     // SpeakString ( "Im in Lightning breath" ,  TALKVOLUME_SHOUT);
   ...
   if (GetTag(OBJECT_SELF)=="pc_StormDragon")
     {
        nDamage = 800;
        nDC = 80;
     }
...
// Check for Dragon armor and other player items
    ...
    while (GetIsObjectValid(oTarget) && nCnt < nAge)
    {
        if(oTarget != OBJECT_SELF && !GetIsReactionTypeFriendly(oTarget))
        {
            ...
            if(nDamStrike > 0)
            {
                ...
                if (GetTag(OBJECT_SELF)=="pc_RiftDragon" ) //
                    {
                        ePos = EffectDamage(400, DAMAGE_TYPE_POSITIVE);
                        DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, ePos, oTarget));
                    }

                if (GetTag (OBJECT_SELF)=="boom_4m_RiftBoss")
                    {
                        effect ePos = EffectDamage(500, DAMAGE_TYPE_POSITIVE);
                        DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, ePos, oTarget));
                    }
            }
        }
