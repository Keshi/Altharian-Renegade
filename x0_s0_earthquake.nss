...
// End of Spell Cast Hook
    ...
    if (nCasterLvl > 10)
    {
        nCasterLvl = ((nCasterLvl - 10) / 3) + 10;
    }
    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eShake, OBJECT_SELF, RoundsToSeconds(2));
...
//            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                nDamage = MaximizeOrEmpower((6 + nMod), nCasterLvl,  GetMetaMagicFeat());
                //Adjust the damage based on the Reflex Save, Evasion and Improved Evasion. (Don't bother for caster)
                if (oTarget != oCaster)
                {
                    nDamage = GetReflexAdjustedDamage(nDamage, oTarget, (GetSpellSaveDC()+nMod), SAVING_THROW_TYPE_ALL);
                }
