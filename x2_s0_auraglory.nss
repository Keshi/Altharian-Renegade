...
// End of Spell Cast Hook
    ...
    effect eChar = EffectAbilityIncrease(ABILITY_CHARISMA, nMod);

    effect eDur = EffectVisualEffect(VFX_DUR_CESSATE_POSITIVE);
    effect eLink = EffectLinkEffects(eChar, eDur);

    int nDuration = nLevel; // * Duration 1 turn/level
    ...
    if (nMod < 6) nMod = 6;
    effect eFear = EffectSavingThrowIncrease(SAVING_THROW_ALL, nMod, SAVING_THROW_TYPE_FEAR);
    effect eHeal = EffectHeal(MaximizeOrEmpower(10,nMod, nMetaMagic));
