...
// End of Spell Cast Hook
    ...
    if (nCasterLvl > 25)
    {
        nCasterLvl = 25 + ((nCasterLvl - 25)/3);
        nNormMax = (nCasterLvl/5) + 1;
    }
        ...
        if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF))
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                if (GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD)
                {
                    ...
                    nDamage = MaximizeOrEmpower(nDice, nCasterLvl, nMetaMagic);
                }
                else
                {
                    nDamage = MaximizeOrEmpower(nDice, nNormMax, nMetaMagic);
               }
                ...
                if (GetAppearanceType(oTarget) == APPEARANCE_TYPE_VAMPIRE_MALE || GetAppearanceType(oTarget) == APPEARANCE_TYPE_VAMPIRE_FEMALE || GetStringLowerCase(GetSubRace(oTarget)) == "vampire" )
                {
                    ...
                    if (!ReflexSave(oTarget, (GetSpellSaveDC() - 6 + nNormMax), SAVING_THROW_TYPE_SPELL))
                    {
