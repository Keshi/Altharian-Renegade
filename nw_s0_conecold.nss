...
// End of Spell Cast Hook
    ...
    if (nStaff == 4) nLevel = GetEffectiveCasterLevel(OBJECT_SELF);
     ...
     //            if((GetSpellId() == 340 && !GetIsFriend(oTarget)) || GetSpellId() == 25)
            {
                ...
                if(!MyResistSpell(OBJECT_SELF, oTarget, fDelay) && (oTarget != OBJECT_SELF))
                {
                    //Detemine damage
                    nDamage = d6(nLevel);
                    if (nStaff == 2 || nStaff == 4) nDamage = d8(nLevel);
                    //Enter Metamagic conditions
                    if (nMetaMagic == METAMAGIC_MAXIMIZE)
                    {
                         nDamage = nDice * nLevel;//Damage is at max
                    }
