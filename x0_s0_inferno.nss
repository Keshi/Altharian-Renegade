...
void RunImpact(object oTarget, object oCaster, int nMetaMagic)
{
    ...
    if (GetIsDead(oTarget) == FALSE)
    {
        ...
        int nDice = 6;
        int nRoll = 2;
        int nWhite = GetWhiteGold(oCaster);

        if (nWhite == 1 || nWhite == 4)
          {
            nDice = 10;
            nRoll = 4;
          }
        if (nWhite >= 3)
          {
            int nCasterLvl = GetEffectiveCasterLevel(oCaster);
            if (nCasterLvl > 40) nRoll = (nCasterLvl - 40)/20 + 4;
          }
        int nDamage = MaximizeOrEmpower(nDice,nRoll,nMetaMagic);
