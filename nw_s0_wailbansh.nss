...
// End of Spell Cast Hook
    ...
    if (nStaff >=3)
    {
        ...
        int nRange = (nLevel-10) / 5 + 10;
        fRange = IntToFloat(nRange);
    }
    int nToAffect = nLevel;
    object oTarget;
    ...
    while (nCnt <= nToAffect)
    {
        ...
        if(GetIsObjectValid(oTarget) && fTargetDistance <= fRange)
        {
            if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF))
            {
                ...
                if(!MyResistSpell(OBJECT_SELF, oTarget)) //, 0.1))
                {
                    ...
                    if(!MySavingThrow(SAVING_THROW_FORT, oTarget, nSpellDC, SAVING_THROW_TYPE_DEATH)) //, OBJECT_SELF, 3.0))
                    {
