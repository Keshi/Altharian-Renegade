...
// End of Spell Cast Hook
      ...
      if (HasItem(OBJECT_SELF,"toaa_staffeterna")) nStaff = 2;
      if (HasItem(OBJECT_SELF,"avanar_staff")) nStaff = 4;
      if (nStaff > 0)
      {
      ...
      if (GetIsObjectValid(oTarget))
      {
            if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
            {
                ...
                if ( GetCurrentHitPoints(oTarget) <= nMax/2 )
                {
            ...
            while (nDamageDealt < nMax)
            {
                  ...
                  while (GetIsObjectValid(oTarget))
                  {
                        ...
                        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
                        {
                              ...
                              if ((nHitpoints < nMin) && ((nHitpoints > 0) && (nHitpoints <= nMax/9)) && bKill == FALSE)
                              {
                                    nMin = nHitpoints;
                                    oWeakest = oTarget;
                              }
