...
void RunHandImpact(object oTarget, object oCaster, int nVariable)
{
    ...
    if (nVariable > 2) nCasterModifiers = GetCasterAbilityModifier(oCaster) + GetEffectiveCasterLevel(oCaster);
    ...
    if (nCasterRoll >= nTargetRoll)
    {
       ...
       if (nVariable >= 1 && nVariable != 2)
       {
          int nBonus = Random(GetEffectiveLevel(oCaster)) + 13;
          nDam = MaximizeOrEmpower(8, 2, GetMetaMagicFeat(), nBonus);
       }
    ...
    */
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if(nResult  == 0)
        {
            ...
            RunHandImpact(oTarget,OBJECT_SELF,nStaff);
