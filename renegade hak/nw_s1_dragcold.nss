...
void main()
{
    ...
    int nDC;// altharian modifier = nAge / 2;
    int nDamageDice = 10;
    ...
    else if (nAge > 37) //Great Wyrm
    {
     ...
     if (GetTag(OBJECT_SELF)=="pc_IceWyrm")
     {
        nDamage = 200;
        nDC = 30;
     }
   ...
   if (GetTag(OBJECT_SELF)=="pc_GreaterIceWyrm")
     {
        nDamage = 800;
        nDC = 80;
     }
...
ColdBreath = CreateBreath(OBJECT_SELF, FALSE, 40.0, DAMAGE_TYPE_COLD, nDamage, nDamageDice, ABILITY_CONSTITUTION, nDC);

SpeakString(IntToString(nDamage));
SpeakString(IntToString(nDamageDice));
