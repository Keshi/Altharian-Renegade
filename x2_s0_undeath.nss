...
void DoUndeadToDeath(object oCreature, int nMod)
{
    SignalEvent(oCreature, EventSpellCastAt(OBJECT_SELF, GetSpellId()));
    SetLocalInt(oCreature,"X2_EBLIGHT_I_AM_DEAD", TRUE);

    if (!MySavingThrow(SAVING_THROW_WILL,oCreature,(GetSpellSaveDC() + nMod),SAVING_THROW_TYPE_NONE,OBJECT_SELF))
    {
...
// End of Spell Cast Hook
    ...
    int nLevel = GetCasterLevel(OBJECT_SELF);
     ...
     if (nLevel>20)
     {
         nLevel = 20 + ((nLevel - 20) /4);  // Normal modification for non-Vesper.
     }
    ...
    if (nVesper == 2 || nVesper == 4)
     ...
     int nLow = 9999;
     object oLow;
     int nHDLeft = d4(nLevel) + (nLevel*nMod);
    //Enter Metamagic conditions
    if (nMetaMagic == METAMAGIC_MAXIMIZE)
    {
        nHDLeft = (4+nMod) * nLevel;//Damage is at max
    }
     ...
     if  (GetIsObjectValid(oFirst))
     {
        ...
        while (GetIsObjectValid(oTarget) && nHDLeft >0)
        {
            ...
            if (!GetIsObjectValid(oTarget))
            {
                ...
                {
                    DoUndeadToDeath(oLow, (nLevel/10));
