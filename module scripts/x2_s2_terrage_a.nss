...
if( GetTag(oHelm)=="helmofnorthernfury")
  {
    nDuration = nDuration *2 ;    //doubles the fear effects duration
    nBonusHD =  (GetLevelByClass(CLASS_TYPE_BARBARIAN) - 14) /5 ;
  }
//RJ
          ...
          if(!MySavingThrow(SAVING_THROW_WILL, oTarget, nDC, SAVING_THROW_TYPE_FEAR))
            {
                ...
                if (GetHitDice(oTarget)< GetHitDice(oBarb)+ nBonusHD)
                {
                ...
                else if (GetHitDice(oTarget)< (GetHitDice(oBarb)+nBonusHD)*2)
                {
