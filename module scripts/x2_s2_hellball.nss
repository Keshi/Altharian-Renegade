...
void main()
{
    ...
    */
    ...
    if (GetObjectType(oCaster) == OBJECT_TYPE_ITEM)
    {
    ...
    int nTotalDamage;
    int nRoll = 10;
    int nHellUses = GetLocalInt(OBJECT_SELF,"Hellcycles");
    if (nHellUses >= 3) nHellUses = 3;
    while (GetIsObjectValid(oTarget))
    {
        ...
        fDelay = GetDistanceBetweenLocations(lTarget, GetLocation(oTarget))/20 + 0.5f;
          ...
          int iMage= GetLevelByClass (CLASS_TYPE_WIZARD,OBJECT_SELF)+ GetLevelByClass (CLASS_TYPE_SORCERER,OBJECT_SELF)+ GetLevelByClass (CLASS_TYPE_CLERIC, OBJECT_SELF)+ GetLevelByClass (CLASS_TYPE_DRUID, OBJECT_SELF);
    ...
    // Caveats for multiple hellball uses
            if (nHellUses == 1) nRoll = nRoll / 2;
            if (nHellUses >= 2) nRoll = nRoll / 4;
            if (nHellUses >= 3)
            {
              nDice = 0;
              sMessage = "You have cracked the foundation of elements.";
            }
    // End caveats for multiple uses

           //Roll damage for each target

            nDamage1 = (Random(nRoll)+1)*(nDice);
            nDamage2 = (Random(nRoll)+1)*(nDice);
            nDamage3 = (Random(nRoll)+1)*(nDice);
            ...
            eDam4 = EffectDamage(nDamage4, DAMAGE_TYPE_COLD);
    ...
    SetLocalInt(OBJECT_SELF,"hellcycles",nHellUses+1);
