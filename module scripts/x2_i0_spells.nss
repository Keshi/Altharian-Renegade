...
void DoMindBlast(int nDC, int nDuration, float fRange)
{
    ...
    {
        //SpeakString ("Place 0",TALKVOLUME_SHOUT);
        ...
        //----------------------------------------------------------------------
        if (nApp == 413 ||nApp== 414 || nApp == 415 ||nApp == 470)
        {
            bImmune = TRUE;
        }
        ...
        {
           // SpeakString ("Place 1",TALKVOLUME_SILENT_SHOUT);
            SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, GetSpellId()));
            fDelay = GetDistanceBetween(OBJECT_SELF, oTarget)/20;
            // already stunned
            if (GetHasSpellEffect(GetSpellId(),oTarget))
            {
              //    SpeakString ("Place 2",TALKVOLUME_SHOUT);
            ...
            {
              //   SpeakString ("Place 3",TALKVOLUME_SHOUT);
                ...
                ////Red's code to increase damage
                int nDamage;
                if (GetTag(OBJECT_SELF)=="pc_BlastDemon")
                    nDamage =100;
                if (GetTag(OBJECT_SELF)=="boom_1n_GBlastDem")
                    nDamage =300;
                if (GetTag(OBJECT_SELF)=="boom_2n_GBlastDem2")
                    nDamage =500;
                    ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(nDamage,DAMAGE_TYPE_NEGATIVE), oTarget);
            //               SpeakString ("Place 4",TALKVOLUME_SHOUT);
