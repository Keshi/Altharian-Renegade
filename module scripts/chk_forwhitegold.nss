...
int StartingConditional()
{
    ...
    int nPallyLevel = GetLevelByClass(CLASS_TYPE_DRUID, oPC);
    if(nPallyLevel >= 5) return TRUE;
    nPallyLevel = GetLevelByClass(CLASS_TYPE_RANGER, oPC);
    if(nPallyLevel >= 5) return TRUE;
    nPallyLevel = GetLevelByClass(CLASS_TYPE_SHIFTER, oPC);
    if(nPallyLevel >= 5) return TRUE;
