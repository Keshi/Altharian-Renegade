...
void main()
{
    ...
    int nLevel = GetEffectiveLevel(oCaster);
    if (nLevel > 40) nCasterLvl = (nCasterLvl * nLevel / 40);
    int nDamage;
    float fDelay;
    int nMod = (nCasterLvl / 20) + 2;
    ...
    while (GetIsObjectValid(oTarget))
    {
        if(spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) == TRUE)
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                nDamage = d6(nCasterLvl) + 2*(nCasterLvl);
                //Resolve metamagic
                //Adjust the damage based on the Reflex Save, Evasion and Improved Evasion.
                nDamage = GetReflexAdjustedDamage(nDamage, oTarget, (GetSpellSaveDC()+nMod), SAVING_THROW_TYPE_FIRE);
