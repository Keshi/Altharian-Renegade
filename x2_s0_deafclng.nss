...
void  AddDeafeningClangEffectToWeapon(object oMyWeapon, float fDuration, int nBonus)
{
   IPSafeAddItemProperty(oMyWeapon,ItemPropertyAttackBonus(1+nBonus), fDuration, X2_IP_ADDPROP_POLICY_KEEP_EXISTING ,TRUE,TRUE);
   if (nBonus <= 0) IPSafeAddItemProperty(oMyWeapon,ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_SONIC, IP_CONST_DAMAGEBONUS_3), fDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING ,FALSE,TRUE);
   if (nBonus > 0) IPSafeAddItemProperty(oMyWeapon,ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_SONIC, IP_CONST_DAMAGEBONUS_2d12), fDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING ,FALSE,TRUE);
...
// End of Spell Cast Hook
    ...
    if(GetIsObjectValid(oMyWeapon) )
    {
        ...
        if (nDuration>0)
        {
            ...
            AddDeafeningClangEffectToWeapon(oMyWeapon, RoundsToSeconds(nDuration),nMod);
    }
        return;
    }
        else
