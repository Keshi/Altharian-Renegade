...
string   sSpawn = GetLocalString(OBJECT_SELF,"SPAWN_STR");
 ...
 //    }
    ...
    int iHowMany  = StringToInt(GetSubString(sSpawn,10,2));
    if (iHowMany > 1)
    {
        int iWaypoint = Random (iHowMany)+1;
        string sTag = GetTag(OBJECT_SELF);
        string sWaypoint= sTag+IntToString(iWaypoint);
        location lNewLoc= GetLocation(GetObjectByTag(sWaypoint));
        ActionJumpToLocation (lNewLoc);
    }
}
